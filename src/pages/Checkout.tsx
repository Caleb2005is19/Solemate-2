...
export function Checkout() {
  ...
  const { addOrder, currentUser, userProfile, featureToggles, setIsAuthModalOpen } = useStore();
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await fetch(`/api/coupons/validate?code=${encodeURIComponent(formattedCode)}`);
      ...
      if (res.ok && body.success) {
        ...
        setCouponSuccess(`Coupon "${body.code}" applied! ${discountPercentage}% discount added.`);
      } else {
        setCouponError(body.error || 'Invalid coupon code. Try SOLE20 or WELCOME10');
      }
    } catch (err: any) {
      console.warn('Coupon lookup API failed:', err.message);
      // Client-side hardcoded fallback if server is unreachable
    ...
    return (
      <div className="min-h-[75vh] flex flex-col items-center justify-center p-6 text-center max-w-md mx-auto my-12 animate-in fade-in zoom-in-95 duration-500 font-sans">
        ...
        <p className="text-zinc-500 mb-8 text-sm leading-relaxed">
          Guest checkout is disabled by the store administrator. Please sign in or create an account to finalize your order details and secure your sneakers.
        </p>
        <button
          type="button"
